...
def video2(frame_func, frame_arguments, filename, fps=5, parallel=True):
    """
    ...
    :param frame_func: function called to generate a frame of the video using a frame_argument
    :param frame_arguments: list of arguments used to generate each frame of the video
    :param filename:
    :param fps:
    :param parallel:
    """
    frame_size = np.shape(frame_func(frame_arguments[0]))
    ...
    if parallel:
        with Pool(1, initializer=set_global_function, initargs=(frame_func,)) as pool:
            frames = pool.map(call_global_function, frame_arguments)
    # OpenCV sometimes crashed with multiprocess!
    else:
        frames = map(frame_func, frame_arguments)
...
def interpolation_video(frame_function,
                        t_param,
                        t_range,
                        frames,
                        fps,
    ...
    """
    function_name = frame_function.__name__

    def frame_func(t):
        return sampling_image(frame_function, image_size=frame_size, **{t_param: t})
    ...
    if preview:
        hl.render_from_array(frame_func(t_range[0]))
        hl.render_from_array(frame_func((t_range[0] + t_range[1]) / 2.0))
...
def sampling_image(value_function,
    ...
    :param x_range: Float 2tuple. Range of x to evaluate the function over
    :param y_range: Float 2tuple. Range of y to evaluate the function over
    :param resolution: Scalar or 2tuple. The number of points to sample from range in the x and y directions
        defaults to image_resize
    ...
    """
    print("Creating image with res:{} {}".format(resolution, params))
    ...
    x_axis = hl.np.linspace(start=x_range[0], stop=x_range[1], num=int(resolution_x))
    y_axis = hl.np.linspace(start=y_range[0], stop=y_range[1], num=int(resolution_y))
