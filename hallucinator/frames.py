...
def regional_gradient_frame(f, p, region, x_range, y_range, resolution=5, white_ref=1.0,
                            black_ref=-1.0, default=hallucinator.BLUE, backdrop='new'):
    """
    ...
    gradient_image = hallucinator.set_to_gradient(points=gradient_points,
                                                  x_range=x_range,
                                                  y_range=y_range,
                                                  black_ref=black_ref,
                                                  white_ref=white_ref,
...
def gradient_frame(f, p, x_range, y_range, white_ref=-1.0, black_ref=1.0, resolution=5):
    ...
    phasegrid = hallucinator.phasegrid(lambda a, b: f(a, b, **p)[2],
                                       x_range=x_range,
                                       y_range=y_range,
                                       resolution=resolution)
    grad = hallucinator.arr_to_gradient(phasegrid,
                                        white_ref=white_ref,
                                        black_ref=black_ref)
    return grad
