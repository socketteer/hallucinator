...
def zoneplate(x, y, wavelength):
    return squarewave(math.sqrt(x ** 2 + y ** 2) ** 2 / wavelength**2)
...
def make_zoneplate(x_range, y_range, x_density, y_density, wavelength):
    pts = sample(lambda a, b: zoneplate(a, b, wavelength), x_range, y_range, x_density, y_density)
    canv = hl.set_to_gradient(pts,
                              x_range=x_range,
                              y_range=y_range,
                              black_ref=-1,
...
canv = make_zoneplate(x_range, y_range, x_density, y_density, wavelength)
hl.render_from_array(canv)
hl.save_img(canv, 'zoneplates/discrete-zoneplate_xd{2}-yd{3}-w{4}'.format(x_range,
                                                                           y_range,
                                                                           x_density,
                                                                           y_density,
                                                                           wavelength))
