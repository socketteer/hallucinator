...
def frame_to_image(frame, x_range=(-1000, 1000), y_range=(-1000, 1000),
                   foreground=render.WHITE, background=render.BLACK):
    points = set()
    for obj in frame.objects:
        points = points.union(discretizer.obj_to_set(obj))
    return render.set_to_bichrome(points, x_range=x_range, y_range=y_range,
...
class Scene:
    ...
    def discr_render(self, x_range=(-1000, 1000), y_range=(-1000, 1000),
        ...
        arr = frame_to_image(self, x_range, y_range, foreground, background)
        if display:
